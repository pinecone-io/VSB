...
def rewrite_file(src: Path, dst: Path, expected_dim: int | None, normalize: bool):
    ...
    if val_col is None:
        ...
        if "element" in tbl.column_names:
            raise RuntimeError(
                f"{src}: saw 'element' leaf but no top-level 'values'. "
    ...
    if val_col is None:
        if "element" in tbl.column_names:
            raise RuntimeError(
                f"{src}: saw leaf 'element' but no top-level 'values' — read with PyArrow, not pandas."
            )
        raise RuntimeError(f"{src}: missing required 'values' column")
    ...
    if normalize:
        ...
        vals = pa.ListArray.from_arrays(
            vals.offsets, child, type=pa.list_(pa.float32())
        )
    ...
    if expected_dim is not None:
        ...
        if len(nn) > 0:
            if len(nn[0]) != expected_dim:
                raise RuntimeError(
                    f"{src}: values dim {len(nn[0])} != expected {expected_dim}"
                )
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Rewrite Parquet files for Pinecone bulk import"
    )
    ap.add_argument("--input", required=True, help="Input file or directory")
    ap.add_argument("--output", required=True, help="Output directory")
    ap.add_argument(
        "--expected-dim", type=int, default=None, help="Assert vector length N"
    )
    ap.add_argument(
        "--normalize", action="store_true", help="Normalize uint8 → float32 in [0,1]"
    )
    args = ap.parse_args()
